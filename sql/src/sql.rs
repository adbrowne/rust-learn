...
fn interpret(expr: &Expression) -> i32 {
    match expr {
        ...
        Expression::Function(FunctionCall { name, arguments }) => match name.as_str() {
...
mod tests {
    ...
    fn plus_function_is_evaluated() {
        let result = interpret(&Expression::Function(FunctionCall {
    ...
    fn expression_strategy() -> impl Strategy<Value = Expression> {
        let leaf = prop_oneof![arb_integer_value().prop_map(Expression::Integer),];

        leaf.prop_recursive(8, 256, 10, |left| {
            prop_oneof![
                prop::collection::vec(left.clone(), 2).prop_map(|arguments| Expression::Function(
                    FunctionCall {
                        name: String::from("plus"),
                        arguments
                    }
